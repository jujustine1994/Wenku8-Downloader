...
14. **GUI 驗收進度（2026-09-21）**
    ...
    - ✅ **視覺層面由使用者判定通過**（2026-09-21）。

      但證據強度跟上面幾條不同，寫清楚免得日後誤會：使用者開過視窗、
      覺得沒問題，**沒有逐項目視檢查**排版、間距、捲動、高 DPI 表現。
      這類東西也不值得為了留紀錄而特地去測——真的醜或難用，用的人自然會發現。

      **這條的用途是：日後如果版面出問題，先懷疑這裡。** 它是整個
      2026-09-21 這輪改動中唯一沒有硬證據的部分，其餘都有測試或實測數字撐著。
      會動到版面的改動（新增 widget、改 grid/pack 參數、動 THEMES 的字型或
      間距）尤其要留意。
