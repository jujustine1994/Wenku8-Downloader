...
14. **GUI 部分驗收（2026-09-21）**：使用者開過視窗，版面與新按鈕沒有問題。
    但**下列互動路徑仍未確認實際跑過**，都只有 headless smoke test 與資料層驗證：
    - 按「更新」→ 六組分類確認視窗（動態組出來的，變數最多）
    - 載入已有檔案的資料夾 → 「已有 1–10 卷」提示與兩個選擇鈕
    - 貼單卷網址 → Preview 只顯示一卷、確認後編號仍照整份目錄算
    下次實際用到時留意，出問題再回報
