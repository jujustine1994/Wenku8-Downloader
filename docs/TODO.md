...
7. **`src/main.py` 還有 142 條寫死中文要搬 `t()`**（批次 3 未完成）。母表
...
11. **`_enable_wheel_scroll()` 的 `bind_all` handler 洩漏**（非 i18n 範圍，
    ...
    沒比照辦理
12. **`show_cth_banner()` 直接 print ANSI escape**（順手發現）：非 tty
